...
>>1. lock方法中，不会让线程直接做cas操作，没有这一步判断：if (compareAndSetState(0, 1)), 直接调用尝试获得锁的方法，也就是说，公平锁不能根据当前状态是不是0来判定，你这个线程能不能拿到锁。
...
#### 3. 等待对立中的线程怎么办？
回观acquire方法，其实公平锁和非公平锁都有调用，在lock方法体中，源码再看一遍，如下
```java
public final void acquire(int arg) {
    if (!tryAcquire(arg) &&
        acquireQueued(addWaiter(Node.EXCLUSIVE), arg))
...
> 虽然公平锁和非公平锁的等待队列的出队顺序一样，“公平”与“非公平”的主要区别在于，在state==0时，如果等待队列中有等待线程，如果有新的线程执行lock操作，这时，如果是公平锁，必须将该线程放到队列中排队，等排在前面的拿到锁之后他才能拿到锁。但是非公平锁则不然，他【有可能】比等待队列中的线程先得到所。